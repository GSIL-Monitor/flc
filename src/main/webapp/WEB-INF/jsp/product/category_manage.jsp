...
<div class="table_menu_list" id="testIframe">
    <table class="table table-striped table-bordered table-hover" id="sample-table">
        ...
        <tbody>
        <c:forEach items="${classifyListByLevelOne}" var="classify">
            <tr>
                <td width="80px"><a
                        href="${pageContext.request.contextPath }/classify/getclassifybypid?parentId=${classify.id}&code=2&classifyPLevel=${classify.classifyLevel}"
                        title="查看子分类">${classify.classifyName}</a></td>
                <td width="80px">
                ...
                <c:if test="${classify.classifyType ==1}">
                    <td class="td-status"><span class="btn btn-xs btn-success">显示</span></td>
                </c:if>
                ...
                <td class="td-manage">
                    ...
                    <c:if test="${classify.classifyIndex==1}">
                        <span class="tuijian">
                        <a title="点击取消推荐" onclick="quxiaotuijian(this,${classify.id},${classify.classifyIndex})" class="btn btn-xs btn-success"><i class="icon-beer bigger-120"></i></a></span>
                    </c:if>

                    <c:if test="${classify.classifyIndex==0}">
                    <span class="tuijian">
                        <a title="点击加入推荐" onclick="tuijian(this,${classify.id},${classify.classifyIndex})" class="btn btn-xs"><i class="icon-beer bigger-120"></i></a></span>
                    </c:if>
        ...
        <div class="add_adverts">
            <ul>
                ...
                <li>
                    ...
                    <span class="cont_style">
                    <label>
                        <input name="classifytype" type="radio" checked="" class="ace" id="classifytype1" value="1">
        ...
        <div class="add_adverts">
            <ul>
                <li>
                    ...
                    <%--上级分类Id  隐藏域--%>
                    <input type="hidden" value="" name="classifyPId" id="classifyPId"/>
                    <%--上级分类等级  隐藏域--%>
                    <input type="hidden" value="" name="classifyPLevel" id="classifyPLevel"/>
                    <span class="cont_style" name="classifyPName" id="classifyPName"></span>
                </li>
                <li>
                    <label class="label_name">分类名称:</label>
                    <span class="cont_style">
...
<script type="text/javascript">
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                ...
                if ($('#classifyName').val() === "") {
                    ...
                    $.ajax({
                        type: "post",
                        url: "${pageContext.request.contextPath}/classify/addclassify",
                        data: form,
                        async: false,
                        contentType: false,
                        processData: false,
    ...
    function classifyson(obj, pid, pname, plevel) {
        /*显示父类分类名*/
        $('#classifyPName').html(pname);
        $('#classifyPId').val(pid);
        $('#classifyPLevel').val(plevel);
    ...
    function member_del(obj, id) {
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/classify/delectoneclassify",
            data: {"id": id},
            dataType: "json",
            success: function (data) {
                if (data.code == 2000) {
    ...
    /*取消推荐状态*/
        ...
        function quxiaotuijian(obj, id,indexs) {
        layer.confirm('确认取消推荐吗？', function (index) {
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath}/classify/tuijian",
                data: {"id": id,"classifyIndex": indexs},
                dataType: "json",
                success: function (data) {
                    if (data.code == 2000) {

                        $(obj).parents("td").find(".tuijian").prepend('<a onClick="tuijian(this,'+id+','+indexs+')" title="点击加入推荐"  class="btn btn-xs"><i class="icon-beer bigger-120"></i></a>');
                        $(obj).remove();
    ...
    /*改为推荐状态*/
    function tuijian(obj, id,indexs) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.code == 2000) {
                        ...
                        $(obj).parents("td").find(".tuijian").prepend('<a onClick="quxiaotuijian(this,'+id+','+indexs+')" title="点击取消推荐" class="btn btn-xs btn-success"><i class="icon-beer bigger-120"></i></a>');
                        $(obj).remove();
