...
<div class="margin clearfix">
    <div class="detailed_style clearfix">
        <em class="type"></em>
        <div class="shop_logo">
            <img src="${store.storeLogo}" style="width: 165px;height: 100px;"/>
        </div>
    ...
    <%----%>
    <div class="At_button">
        <button onclick="through_save('this',${store.id});" class="btn btn-primary radius" type="submit">通 过</button>
        <button onclick="cancel_save('this',${store.id});" class="btn btn-danger  btn-warning" type="button">拒 绝</button>
...
<script type="text/javascript">
    ...
    function through_save(obj, id) {
        layer.confirm('确认要开通该店铺吗？', function (index) {
            ...
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath}/store/modifystatuspass",
                data: {"id": id},
                dataType: "json",
                success: function (data) {
                    if (data.code == 2000) {
                        layer.msg('已开通!', {icon: 1, time: 1000});
                        location.href = "${pageContext.request.contextPath}/store/getstoreauditlist";
        ...
        var index = layer.open({
            ...
            yes: function (index, layero) {
                if ($('.form-control').val() == "") {
                    ...
                    $.ajax({
                        type: "post",
                        url: "${pageContext.request.contextPath}/store/modifystatusreject",
